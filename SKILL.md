...
- 深度访谈两份报告的概况表首列固定为“序号”、第二列固定为“姓名”；序号按 `selectedPatients` 入选顺序从 1 连续编号，姓名取对应患者主表姓名。逐人标题、正文及引语署名继续使用姓名替代 P01、P02 等患者编号，完整患者ID保留用于核验；每位患者的逐人访谈表字段和两列结构不变。姓名映射与缺失处理见访谈流程。
...
适用于：`生成不良反应清单 依据文件：/path/月度患者清单.xlsx 产品：血栓通胶囊 服务周期：2026-08-01 至 2026-08-15`。
...
3. 将用户提供的服务周期拆成开始日期和结束日期，运行：

   ```bash
   python scripts/generate_adverse_reactions.py \
     --patients patients.json --product "产品名称" \
     --service-start 2026-08-01 --service-end 2026-08-15 \
...
- `服务周期`：必填，格式为 `YYYY-MM-DD 至 YYYY-MM-DD`；对应脚本 `--service-start` 和 `--service-end`，保存到 `meta.servicePeriod.start/end`。缺失时先要求补充，不得从激活月份或当前日期推断；非法日期或开始日晚于结束日时停止生成。
