...
13 列格式的 `inputFormat` 为 `medicationReminder13`，14 列格式为 `medicationReminder14`，15 列格式为 `medicationReminder15`。三者都没有激活日期，`用药方案确认时间` 是唯一的确认时间来源，必须非空且可解析；提取结果保留 `sourceConfirmationTime` 和 `confirmationTime`，并将 `activateTime` 置为空。不得读取或解析旧 `手术名称`、`耗材名称`、`联合用药`、`用药方案` 来补造临床事实，手术名称和耗材名称必须根据本次公司、产品类型、产品名称与患者疾病重新生成。
...
- `用药提醒_<产品名称>.xlsx`：每个 userid 一行，固定为上述 15 列。G 列“既往过敏史”后紧接 H 列“手术名称”和 I 列“耗材名称”，原联合用药及后续字段顺延至 J:O；标题合并范围、表格对象、格式和预览均覆盖 A:O。H 列逐行取对应 `records[].surgeryName`。当公司为 `商联医药(河南)有限公司（器械）` 且产品类型为器械时，I 列固定填写当前产品名称，否则留空。最终验证器核对实际 H/I 列与 payload 一致，并验证后续字段没有错位。
...
- 13/14/15列“用药提醒”不等同于含体温监测次数、患者响应率的“跟踪提醒”；当前10列“用药方案”没有用药方案确认时间，不能代替任何需要带日期记录的角色；洞察月度模式按表头读取用药提醒表，以明确药名和确认时间进行登记统计，新增“手术名称”和“耗材名称”不改变其他字段含义，不套用旧跟踪提醒或逐药清单字段。缺少必填角色或日期时补充对应原始资料，不推测或补造统计字段。
